...
class ImportMultipleObjs(bpy.types.Operator, ImportHelper):
    ...
    translate_origin: BoolProperty(
            name = "Translate to Origin",
            default=True
            )

    set_name_to_filename: BoolProperty(
            name = "Set Name to Filename",
            default=True
            )
    ...
    def draw(self, context):
        ...
        row = box.row()
        row.prop(self, "translate_origin")
        row = box.row()
        row.prop(self, "set_name_to_filename")
    ...
    def execute(self, context):
        ...
        if hasattr(bpy, 'previous_translation'):
            previous_translation = getattr(bpy, 'previous_translation')
        else:
            previous_translation = None

        
        all_obj_pre = set(bpy.data.objects)
        ...
        for j, i in enumerate(self.files):
            ...
            obj_post = set(bpy.data.objects)
            new_obj = list(obj_post - obj_pre)[0]
            if self.set_name_to_filename:
                fname = os.path.basename(path_to_file)
                for idx, new_obj in enumerate(list(obj_post - obj_pre)):
                    if idx > 0:
                        fnamei = fname + '_' + str(idx+1)
                    else:
                        fnamei = fname
                        
                    new_obj.data.name = fname
                    new_obj.name = fname
        ...
        all_obj_post = set(bpy.data.objects)
        
        if self.translate_origin:
            new_objs = all_obj_post - all_obj_pre

            # bpy.context.view_layer.objects.active = obj
            # bpy.ops.object.parent_set(type='OBJECT', keep_transform=True)
            local_bbox_center = Vector([0,0,0])
            for o in new_objs:
                o.rotation_euler = [0,0,0]
                local_bbox_center += 0.125 * sum((Vector(b) for b in o.bound_box), Vector()) / len(new_objs)


            if previous_translation is None:
                previous_translation = local_bbox_center
                setattr(bpy, 'previous_translation', previous_translation)
            else:
                local_bbox_center = previous_translation

            for o in new_objs: 
                o.location -= local_bbox_center
